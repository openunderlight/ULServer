...
int MsServerInfo::StartServers(int servtype)
{
  ...
  DECLARE_TheLineNum;
  int retval = 0;
  unsigned long ips[Lyra::MAX_CLOSING_GAMEDS];
  unsigned short ports[Lyra::MAX_CLOSING_GAMEDS];

  bool first_gamed = true; // used to determine which gamed 
                            // to point players to initially
  ...
  for (int i = 0; i < main_->ServerDB()->NumServers(); ++i) {
    ...
    // if ids differ, continue
    if (_ttoi(main_->HostID()) != _ttoi(main_->ServerDB()->HostID(i))) {
      main_->Log()->Debug(_T("%s: local ID '%s' != server ID '%s'"), method, main_->HostID(), main_->ServerDB()->HostID(i));
      continue;
    }
    // arg1 = 0?
    ...
    if ((servtype == LmServerDBC::ST_GAME) && first_gamed) {
#ifdef GAMED_POINTER
      main_->Log()->Debug(_T("%s: writing new gamed pointer file; IP %s, port %d\n"), method, main_->ServerDB()->HostName(i), main_->ServerDB()->Arg1(i));
      int retval = main_->ServerDB()->WriteNewLoginFile((char*)(main_->ServerDB()->HostName(i)), main_->ServerDB()->Arg1(i));
#else
      for (int i=0; i<Lyra::MAX_CLOSING_GAMEDS; i++) {
	ips[i] = 0;
	ports[i] = 0;
      }
      int retval = main_->ServerDB()->WriteNewClosingFile(ips, ports);
#endif
      first_gamed = false;
    }
