...
int MsMain::Init(const TCHAR* root_dir, const char* hostid)
{
  pid_ = getpid();
  _tcsnccpy(hostid_, hostid, sizeof(hostid_));
...
void MsMain::Go()
{
  ...
  if ((sinfo_->StartServers(LmServerDBC::ST_LEVEL) < 0) ||
...
void MsMain::handle_child_death(pid_t childpid, int status)
{
  ...
  if (email_admin) {
    LmUtil::SendMail(_T("masterd@underlight"), serverdbc_->DatabaseAdminEmail(), _T("Underlight: server exit"),  _T("HostName: %s\n")    _T("HostID: %s\n")      _T("Error: return code %d from Underlight server, child %lu\n")     _T("Reason: %s %d\n")     _T("Server type: %c, args: %d %d; restarted: %d\n"),     HostName(), HostID(),
		     status, childpid, death_reason, email_code, srv_type, srv_arg1, srv_arg2, restart);
  }
}
