...
int LmGuildDBC::ReportHeaders(const GMsg_GetReportHdrs& msg, lyra_id_t requestor, lyra_id_t* report_ids, lyra_id_t* goal_ids, lyra_guild_sum* summaries, unsigned short* flags, int hirank)
{
  ...
  if (goalid == 0) { // get new reports 
    ...
    if (hirank > 0) // guardian looking for initiate reports
     _stprintf(query, _T("SELECT report.report_id, report.goal_id, report.summary, report.flags, DATE(creation_time) FROM report, goal WHERE report.goal_id = goal.goal_id AND goal.guild = %u AND goal.rank = %u AND MOD(report.flags,2) = 0 AND (goal.creator = %u OR report.recipient = %u) ORDER BY report.creation_time DESC"), guild, rank, requestor, requestor);
    else // initiate, show only ours 
     _stprintf(query, _T("SELECT report.report_id, report.goal_id, report.summary, report.flags, DATE(creation_time) FROM report, goal WHERE report.goal_id = goal.goal_id AND goal.guild = %u AND goal.rank = %u AND MOD(report.flags,2) = 0 AND report.recipient = %u  ORDER BY creation_time DESC"),  guild, rank, requestor);

  } else { // select a single goal
      if (hirank > 0)
		_stprintf(query, _T("SELECT report_id, goal_id, summary, flags, DATE(creation_time) FROM report WHERE goal_id = %u ORDER BY creation_time DESC"),  goalid);
      else _stprintf(query, _T("SELECT report_id, goal_id, summary, flags, DATE(creation_time) FROM report WHERE goal_id = %u  AND (report.recipient = %u OR report.creator = %u) ORDER BY creation_time DESC"),  goalid, requestor, requestor);
  }
